...
function main(start_index::Int, end_index::Int) 
    ...
    for i in ProgressBar(start_index:end_index)
        ...
        # FermiSurfaceIntegration.contracted_integral!(gamma, arclengths, perimeter, momenta, hamiltonian, temperature, q_squared, prec, umklapp = umklapp)
        ...
        #     writedlm(file, prefactor * transpose(itp_n.(interpolation_arclengths) .* sqrt.(norm.(fv) .* ds)), ",")
        # end
