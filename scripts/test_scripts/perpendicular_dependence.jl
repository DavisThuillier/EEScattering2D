...
function main()
    ...
    full_matrix = Matrix{Float64}(undef, size(momenta))

    integral::SVector{2,Float64} = [0.0,0.0]
    for i in CartesianIndices(momenta)
        k_index = (i[1], i[2])
        integral = FermiSurfaceIntegration.collision_integral((div(perp_num,2), 100), k_index, momenta, energies, dVs, hamiltonian, variance, temperature, q_squared, umklapp = umklapp)
